...
uses
  ...
  Data.DB, FireDAC.Comp.DataSet, FireDAC.Comp.Client, Vcl.DBGrids, edCliente;
...
begin
  ...
  if id = 0 then
    ...
    nuevaTab.Caption := 'Cliente ID ' + id.ToString;

  Clienteframe := TFrEdCliente.Create(nuevaTab);
  Clienteframe.Parent := nuevaTab;
  Clienteframe.Align := alClient;

  frmMain.Pagecontrol1.ActivePageIndex :=
    frmMain.Pagecontrol1.PageCount - 1;

  Clienteframe.cargardatos(id);
end;

procedure TListadoFrameCliente.Loaded;
begin
  inherited;
  // ...
end;

procedure TListadoFrameCliente.EliminarRegistro(id: Integer);
begin
  inherited;
  // ...
end;
