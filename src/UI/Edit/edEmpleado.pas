...
uses
  ...
  Vcl.DBCtrls, Vcl.Mask, Vcl.WinXCtrls, dmConnection, uTranslator, Vcl.Grids,
  Vcl.DBGrids, ListadoDirecciones, ListadoDatosBanco, Vcl.ComCtrls, uFuncionesglobales;

type
  TFrEdEmpleado = class(TFrEdit)
...
procedure TFrEdEmpleado.doSave;
var
  IdEmpleado: Integer;
begin
  ...
  IdEmpleado := DBId.Field.AsInteger;

  if Assigned(Listadodirecciones) then
    Listadodirecciones.cargarlistadoentidad(IdEmpleado, 'EMPLEADO');

  if Assigned(ListadoDatosBanco) then
    ListadoDatosBanco.CargarListadoEntidad(IdEmpleado, 'EMPLEADO');
end;

procedure TFrEdEmpleado.EditPasswordChange(Sender: TObject);
begin
  inherited;
...
procedure TFrEdEmpleado.VincularControles;
var
  I: Integer;
  Ed: TDBEdit;
  FieldName: string;
...
begin
  ...
  FDQuery.SQL.Text := 'SELECT * FROM empleado WHERE 1 = 0';
  FDQuery.Open;
  ...
  if Parent is TTabSheet then
    TTabSheet(Parent).Caption := 'Nuevo Empleado';
end;

Procedure TFrEdEmpleado.CargarPermisos;
begin
...
Function TFrEdEmpleado.ValidarCampos:boolean;
begin
  Result := True;
  ...
  // NO hay tarifa en empleado, así que no se valida
  ...
  if Length(Trim(EditPassword.Text)) < 6 then
  begin
...
Procedure TFrEdEmpleado.Cargardatos(Id: Integer);
begin
...
Procedure TFrEdEmpleado.cargarframedirecciones(ID:integer);
begin
  if Assigned(ListadoDirecciones) then
  begin
  ...
  Listadodirecciones.cargarlistadoentidad(ID, 'EMPLEADO');
end;

Procedure TFrEdEmpleado.cargarframeDatosBanco(ID: Integer);
begin
  if Assigned(ListadoDatosBanco) then
  begin
  ...
  ListadoDatosBanco.CargarListadoEntidad(ID, 'EMPLEADO');
end;

procedure TFrEdEmpleado.cargarListadoDirecciones(id:integer);
begin
  if Id <> 0 then
  begin
